...
struct MapView: View {
    ...
    var body: some View {
        Group {
            GeometryReader { geo in
                if locationVM.allowingLocationTracker {
                    ...
                    ZStack {
                        Map(coordinateRegion: $locationVM.region, interactionModes: .all, showsUserLocation: true, userTrackingMode: $tracking, annotationItems: locationVM.mapAnnotationsList) { pin in
                            MapAnnotation(coordinate: pin.coordinate) {
                                ...
                                Button(action: {
                                    withAnimation(.spring()) {
                                        ...
                                        self.locationVM.region = MKCoordinateRegion(center: CLLocationCoordinate2D(latitude: pin.coordinate.latitude - 0.02, longitude: pin.coordinate.longitude), span: MKCoordinateSpan(latitudeDelta: 0.03, longitudeDelta: 0.03))
        ...
        .task {
            ...
            locationVM.checkIfLocationServicesIsEnabled(limitResults: false)
        }
        .onChange(of: self.sheetObserver.eventDetailData) { newValue in
            withAnimation {
                self.locationVM.region = MKCoordinateRegion(center: CLLocationCoordinate2D(latitude: newValue.coordinate.latitude - 0.02, longitude: newValue.coordinate.longitude), span: MKCoordinateSpan(latitudeDelta: 0.03, longitudeDelta: 0.03))
            }
        }
        .onDisappear {
